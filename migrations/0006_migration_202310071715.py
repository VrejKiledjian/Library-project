...
def forward(old_orm, new_orm):
    bookscopy = new_orm['bookscopy']
    return [
        # Apply default value True to the field bookscopy.is_available,
        bookscopy.update({bookscopy.is_available: True}).where(bookscopy.is_available.is_null(True)),
        # Apply default value '' to the field bookscopy.copy_number,
        bookscopy.update({bookscopy.copy_number: ''}).where(bookscopy.copy_number.is_null(True)),
    ]
...
def backward(old_orm, new_orm):
    bookscopy = new_orm['bookscopy']
    return [
        # Apply default value 0 to the field bookscopy.state,
        bookscopy.update({bookscopy.state: 0}).where(bookscopy.state.is_null(True)),
    ]
